<?php

return[
  "access_request_status" => [
    "approved" => "Goedgekeurd",
    "denied" => "Geweigerd",
    "pending" => "In behandeling"
  ],
  "activity_action" => [
    "archived" => "Gearchiveerd",
    "created" => "Gecreëerd",
    "deleted" => "Verwijderd",
    "ended" => "Beëindigd",
    "restored" => "Hersteld",
    "started" => "Gestart",
    "updated" => "Bijgewerkt"
  ],
  "border_radius" => [
    "full" => "Vol",
    "large" => "Groot",
    "medium" => "Gemiddeld",
    "none" => "Geen",
    "small" => "Klein"
  ],
  "custom_field_type" => [
    "block_editor" => "Blok Editor",
    "color" => "Kleur",
    "date" => "Datum",
    "link" => "Link",
    "markdown" => "Markdown",
    "number" => "Nummer",
    "text" => "Tekst",
    "textarea" => "Tekstveld"
  ],
  "custom_field_type_description" => [
    "block_editor" => "Een geavanceerde editor voor het creëren van rijke content met blokken.",
    "color" => "Een veld om een kleur te kiezen.",
    "date" => "Een veld voor het selecteren van een datum.",
    "link" => "Een veld voor het invoeren van een URL.",
    "markdown" => "Een tekstveld dat Markdown-opmaak ondersteunt.",
    "number" => "Een veld dat alleen numerieke invoer accepteert.",
    "text" => "Een eenvoudig tekstveld voor korte invoer.",
    "textarea" => "Een groter tekstveld voor langere invoer."
  ],
  "data_export_format" => [
    "html" => "HTML",
    "json" => "JSON"
  ],
  "front_type" => [
    "co_con" => "Co-Con",
    "co_front" => "Co-Front",
    "front" => "Front"
  ],
  "layout_preset" => [
    "bold" => "Bold",
    "bold_description" => "Een opvallende lay-out met grote letters en levendige secties",
    "card_grid" => "Card Grid",
    "card_grid_description" => "Members displayed in a card-based grid layout",
    "classic" => "Klassiek",
    "classic_description" => "Een traditionele profielindeling met een lineaire opbouw",
    "minimal" => "Minimaal",
    "minimal_description" => "A clean, distraction-free layout"
  ],
  "member_visibility" => [
    "admin" => "Beheerders en hoger",
    "friend" => "Vrienden en hoger",
    "owner_only" => "Alleen voor eigenaar",
    "partner" => "Partners en hoger",
    "public" => "Iedereen",
    "trusted" => "Vertrouwd en hoger"
  ],
  "notification_description" => [
    "admin_announcement" => "Receive important announcements from system administrators.",
    "data_export_completed" => "Get notified when your data export is ready to download.",
    "data_export_failed" => "Get notified if there was an issue with your data export.",
    "front_changed" => "Ontvang een melding wanneer de front verandert.",
    "invitation_received" => "Ontvang een melding in de app wanneer iemand je uitnodigt om lid te worden van hun systeem.",
    "role_changed" => "Ontvang een melding als je rol of rechten binnen het systeem veranderen.",
    "user_invited" => "Ontvang een melding wanneer je wordt uitgenodigd om lid te worden van een systeem.",
    "user_joined" => "Get notified when a new user joins the system.",
    "user_removed" => "Receive a notification if you are removed from the system."
  ],
  "notification_type" => [
    "admin_announcement" => "Mededeling van de beheerder",
    "data_export_completed" => "Gegevensexport voltooid",
    "data_export_failed" => "Gegevensexport mislukt",
    "front_changed" => "Front Changed",
    "invitation_received" => "Uitnodiging ontvangen",
    "role_changed" => "Role Changed",
    "user_invited" => "User Invited",
    "user_joined" => "User Joined",
    "user_removed" => "User Removed"
  ],
  "poll_status" => [
    "closed" => "Gesloten",
    "closed_description" => "De poll accepteert geen stemmen meer",
    "open" => "Open",
    "open_description" => "Poll is accepting votes"
  ],
  "profile_visibility" => [
    "friends" => "Alleen vrienden",
    "hidden" => "Verborgen",
    "private" => "Privé",
    "public" => "Openbaar"
  ],
  "report_status" => [
    "dismissed" => "Dismissed",
    "pending" => "In behandeling",
    "upheld" => "Upheld"
  ],
  "spacing_scale" => [
    "compact" => "Compact",
    "default" => "Default",
    "spacious" => "Spacious"
  ],
  "system_import_status" => [
    "completed" => "Voltooid",
    "completed_with_errors" => "Voltooid met fouten",
    "failed" => "Mislukt",
    "pending" => "In behandeling",
    "processing" => "In verwerking"
  ],
  "system_mood" => [
    "anxious" => "Anxious",
    "anxious_tagline" => "The headspace is a little uneasy right now.",
    "calm" => "Calm",
    "calm_tagline" => "The headspace is calm and settled.",
    "cozy" => "Cozy",
    "cozy_tagline" => "The headspace feels safe and cozy.",
    "energetic" => "Energetic",
    "energetic_tagline" => "The headspace is buzzing with energy.",
    "foggy" => "Foggy",
    "foggy_tagline" => "The headspace is hazy — take it slow.",
    "happy" => "Happy",
    "happy_tagline" => "The headspace is feeling warm and bright.",
    "stressed" => "Stressed",
    "stressed_tagline" => "The headspace is under some pressure.",
    "tired" => "Tired",
    "tired_tagline" => "The headspace is running low on spoons."
  ],
  "system_permission" => [
    "chat.use" => "Use chat",
    "chat.use_description" => "Send and receive messages in the :system chat",
    "custom-fields.view" => "View custom fields",
    "custom-fields.view_description" => "View custom fields on :member profiles",
    "device-tokens.manage" => "Manage device tokens",
    "device-tokens.manage_description" => "Manage device push notification tokens",
    "front.edit" => ":front Bijwerken",
    "front.edit_description" => "Log and update who is currently :fronting",
    "front.view" => ":front status bekijken",
    "front.view_description" => "See who is currently :fronting",
    "journal.edit" => "Write journal",
    "journal.edit_description" => "Create and edit journal entries",
    "journal.view" => "View journal",
    "journal.view_description" => "Read journal entries up to their permitted tier",
    "members.edit" => "Manage :members",
    "members.edit_description" => "Add, edit, and archive :system :members",
    "members.map" => "View :system map",
    "members.map_description" => "View the :system map showing :member relationships",
    "members.view" => "View :members",
    "members.view_description" => "View :system :members and their profiles",
    "notes.edit" => "Manage notes",
    "notes.edit_description" => "Create, edit, and delete notes",
    "notes.view" => "View notes",
    "notes.view_description" => "View notes created by :system :members",
    "polls.edit" => "Manage polls",
    "polls.edit_description" => "Create, edit, and delete polls",
    "polls.view" => "View polls",
    "polls.view_description" => "View polls and vote on them",
    "system.manage" => "Manage :system",
    "system.manage_description" => "Change :system settings and privacy tiers"
  ],
  "system_user_role" => [
    "admin" => "Beheerder",
    "owner" => "Eigenaar",
    "partner" => "Partner",
    "read_only" => "Vriend",
    "trusted" => "Vertrouwde Vriend"
  ],
  "system_visibility" => [
    "friends" => "Alleen vrienden",
    "private" => "Privé",
    "public" => "Openbaar"
  ]
];